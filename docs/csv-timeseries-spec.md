...
本ドキュメントは、本アプリケーションにおいて  
**時系列折れ線グラフによるデータ可視化**を行うための  
CSVフォーマットおよび内部正規形を定義する。

## 対象データ

- 時系列データ（時間軸に沿って変化する数値データ）

## 可視化方法

- 時系列折れ線グラフ
  - 時間を x 軸、数値を y 軸として表示する
  - entity（対象を表す識別子）による系列の切替
  - 同一 entity 内での複数 metric の同時表示を想定する

## 目的

- ユーザーがExcel等で容易に作成できるCSV形式を提供する
- 内部DBでは拡張性の高い正規形（long format）で保持する
- entity 切替および複数系列（metric）表示に対応する
...
- **形式例**（MVP対象）:
  ...
  - 日時: `2023-01-15T13:45:00Z` など ISO 8601 形式
  - 期間ラベル（非対応）: `Q1 2023`, `2023H1`, `FY2022`
  - 順序ラベル（非対応）: `Step1`, `Stage2`

- **パースルール**:
  1. 元の文字列は `raw_time` としてそのまま保持する。
  2. 年・年-月・日付・ISO 8601 形式の日時は `datetime` に変換し、`time` 列へ格納する。
     - MVPでは **Year / Year-Month / Date / ISO 8601日時** 形式に対応。
     - 期間ラベルや任意ステップは未対応とし、`NULL` として処理。
...
- **MVP対応方針**:
  - Year / Year-Month / Date / ISO 8601日時 形式は対応する。
  - 期間ラベルや任意ステップなど、それ以外の形式は `NULL` として扱う。

### entity（任意・最大1列）

- 可視化において「系列を分ける対象」を表す識別子  
  （例：国、地域、センサー、個体、機器など）
- 同一 entity 内では、複数 metric が同時に表示される
- 文字列として扱う
