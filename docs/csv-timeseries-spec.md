...
- **形式例**:
  - 年のみ: `2020`, `1990.0`
  - 年-月: `2023-01`, `2023/02`
  - 完全日付: `2023-01-15`, `15/01/2023`
  - 日時: `2023-01-15T13:45:00Z`
  - 期間ラベル: `Q1 2023`, `2023H1`, `FY2022`
  - 順序ラベル: `Step1`, `Stage2`

- **パースルール**:
  1. 元の文字列は `raw_time` としてそのまま保持
  2. 年・年-月・日付・日時は `datetime` に変換し、`time` 列へ格納
  3. パース不可能な場合は `time = NULL`
  4. 表示やグラフ描画時は、`time` が存在する場合は `time` 昇順、NULL の場合は CSV 行順（`order_index`）で並べる

- **MVP対応**: まず Year / Year-Month / Date の形式に対応し、その他は NULL として処理
